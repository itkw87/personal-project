...
import personal.project.util.ApplicationContext;
...
import personal.project.util.HttpSession;
...
public class ServerApp {
  ...
  public static final String PERSONAL_SESSION_ID = "personal_session_id";

  ApplicationContext iocContainer;
  DispatcherListener dispatcherServlet;
  Map<String, HttpSession> sessionMap = new HashMap<>(); // 전체 사용자의 Session을 보관하기 위한 Map
  ...
  public ServerApp(int port) throws Exception {
    ...
    dispatcherServlet = new DispatcherListener(iocContainer);
  }
  ...
  private NettyOutbound processRequest(HttpServerRequest request, HttpServerResponse response) {
    ...
    try {
      ...
      List<Cookie> cookies = request2.allCookies().get(PERSONAL_SESSION_ID);
      if (cookies != null) {
        // 세션ID가 있으면 이 값을 가지고 클라이언트를 구분한다.
        sessionId = cookies.get(0).value(); // 일단은 무조건 0번째
      } else {
      ...
      // 세션ID로 클라이언트에게 배정된 HttpSesion 객체를 찾는다.
      HttpSession session = sessionMap.get(sessionId);
      if (session == null) {
        // 현재 클라이언트가 사용할 HttpSession 객체가 배정되 않았다면, 새로 만든다.
      ...
      if (firstVisit) {
        ...
        response.addCookie(new DefaultCookie(PERSONAL_SESSION_ID, sessionId));
      }
