...
public class MySQLMemberDao implements MemberDao {
  ...
  public void insert(Member member) {
    try (
        // Connection con = ds.getConnection(); // Connection 변수를 try ~ with ~ resources안에 선언하면
        // insert한번 할 때 마다 Connection close() 즉, 연결 끊겨서 안됨
  ...
  public Member findBy(int no) {
    try (PreparedStatement stmt = ds.getConnection(false)
        .prepareStatement("select " + " member_no," + " authority," + " birth," + " grade,"
            + " name," + " gender," + " korean_score," + " english_score," + " math_score,"
            + " scoreAvg," + " email," + " status," + " created_date" + " from ps_member "
            + " where member_no=?");) {
  ...
  public int delete(int no) {
    try (PreparedStatement stmt =
        ds.getConnection(false).prepareStatement("delete from ps_member where member_no=?")) {
